...
const editSchema = z.object({
  direccion: z
    .string()
    .min(1, "La dirección de recogida es obligatoria")
    .max(255),
  fecha_programada: z
    .string()
...
const MapCard = memo(
  ({
    ...
    if (
      !querySearch.toLowerCase().includes("españa") &&
      !querySearch.toLowerCase().includes("spain")
    ) {
      querySearch += ", España";
    }
...
const EmpleadoDetailView = memo(
  ({
    ...
    pagoMetodoId,
    setPagoMetodoId,
    handleRegistrarPago: _handleRegistrarPago,
    handlePagar: _handlePagar,
    pagando: _pagando,
    serverError,
    puedeAvanzar,
    handleAvanzarEstado,
    ...
    const scheduledDate = solicitud.fecha_programada
      ? new Date(solicitud.fecha_programada)
      : null;
          ...
          <CardSinBorde className="border border-border shadow-sm h-full rounded-xl overflow-hidden relative">
            <CardContent className="p-6 flex flex-col h-full space-y-6">
              ...
              <div className="space-y-4">
                {["en_itv", "retornando", "finalizado"].includes(
                  solicitud.estado?.slug || "",
                ) && (
                  <div className="space-y-2 p-4 bg-primary/5 rounded-xl border border-primary/10">
                    ...
                    <select
                      ...
                      disabled={
                        solicitud.estado?.slug !== "en_itv" || avanzando
                      }
                      onChange={async (e) => {
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 pt-1">
                    <Textarea
                      ...
                      disabled={
                        avanzando || solicitud.estado?.slug === "finalizado"
                      }
                    />
                    <Button
                      size="sm"
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        value={
                          pagoImporte !== ""
                            ? pagoImporte
                            : solicitud.importe_cobro
                              ? String(solicitud.importe_cobro)
                              : ""
                        }
              ...
              {solicitud.pago &&
                (solicitud.pago.estado_pago?.slug === "pagado" ||
                  solicitud.pago.estado_pago?.id === 2) && (
                  <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-center">
                    <div className="flex items-center justify-center gap-2 text-emerald-700 font-black text-sm uppercase italic">
                      <ShieldCheck size={18} /> Pago completado (
                      {solicitud.pago.metodo_pago?.nombre})
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-3">
                    ...
                    {!puedeGestionarAhora && esAsignado && (
                      <p className="text-center text-amber-600 font-medium text-xs italic">
                        * El servicio está programado para el día{" "}
                        {format(
                          new Date(solicitud.fecha_programada!),
                          "dd/MM/yyyy",
                        )}
                        . Solo puede gestionarse su día.
                      </p>
                    )}
                  </div>
                ) : (
                  ...
                  <div className="space-y-4 border-b-2 border-primary pb-8 sm:border-b-0 sm:pb-0 sm:border-r-2 sm:border-primary sm:pr-8">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="text"
                        {...register("direccion")}
                        className={`border-slate-200 ${errors.direccion ? "border-red-500" : ""}`}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm font-medium">
            ...
            <CardSinBorde>
              <CardContent className="space-y-6 pt-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {role === "administrador" &&
                    ["retornando", "finalizado"].includes(
                      solicitud.estado?.slug || "",
                    ) &&
                    !solicitud.pago && (
                      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
                        <Label className="text-[10px] uppercase tracking-widest text-primary font-black mb-1 block text-center">
                          Registro de Cobro
                        </Label>
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                          ...
                          <div className="space-y-1">
                            ...
                            <select
                              ...
                              value={pagoMetodoId ?? ""}
                              onChange={(e) =>
                                setPagoMetodoId(Number(e.target.value))
                              }
                            >
                              <option value="">Seleccionar</option>
                              <option value="1">Efectivo</option>
                  ...
                  {role !== "empleado" ? (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[12px] font-bold text-primary uppercase tracking-tight mt-0.5">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between p-3 rounded-lg bg-slate-50 text-sm">
                    ...
                    <span className="font-bold">
                      {fmt(solicitud.hora_recogida)}
                    </span>
                  </div>
                  <div className="flex justify-between p-3 rounded-lg bg-slate-50 text-sm">
                    <span>Entrada ITV</span>
                    <span className="font-bold">{fmt(solicitud.hora_itv)}</span>
                  </div>
                  <div className="flex justify-between p-3 rounded-lg bg-slate-50 text-sm">
                    <span>Entrega Final</span>
                    <span className="font-bold">
                      {fmt(solicitud.hora_entrega)}
                    </span>
                  </div>
                </div>
              </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch mt-8">
          <div className="space-y-6">
            ...
            {!solicitud.pago && solicitud.importe_cobro && (
              <div
                className="px-6 py-4 rounded-xl border border-yellow-200 flex items-center justify-between gap-4 h-full"
              ...
              >
                <div>
                  ...
                  <div className="text-2xl font-black text-slate-800">
                    {Number(solicitud.importe_cobro).toLocaleString("es-ES", {
                      style: "currency",
                      currency: "EUR",
                    })}
                  </div>
                  <div className="text-xs text-yellow-700 mt-0.5 italic">
                    Pendiente de cobro
                  </div>
                </div>
              </div>
            ...
            {solicitud.pago && (
              <div
                className="px-6 py-4 rounded-xl border flex items-center justify-between gap-4 h-full"
                style={{
                  background:
                    solicitud.pago.estado_pago?.slug === "pagado"
                      ? "rgb(240 253 244)"
              ...
              >
                <div>
                  ...
                  <div className="text-2xl font-black text-slate-800">
                    {Number(solicitud.pago.importe).toLocaleString("es-ES", {
                      style: "currency",
                      currency: "EUR",
                    })}
                  </div>
                ...
                <span
                  className={`px-4 py-1.5 rounded-full text-xs font-bold ring-1 ${
                    solicitud.pago.estado_pago?.slug === "pagado" ||
                    solicitud.pago.estado_pago?.id === 2
          ...
          <div className="font-medium flex flex-col h-full">
            <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1 mb-1">
              Notas del servicio
            </label>
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 pt-8 mt-2 border-t-2 border-primary font-bold">
          ...
          {puedeAvanzar && (
            <Button
              ...
              onClick={handleAvanzarEstado}
              disabled={
                avanzando || (isBusy && siguiente?.slug === "en_recogida")
              }
            >
              {avanzando
                ? "..."
...
export default function SolicitudDetailPage() {
  ...
  useEffect(() => {
    if (editando && solicitud) {
      reset({
        ...
        user_empleado_id: solicitud.empleado?.id ?? undefined,
        notas: solicitud.notas ?? "",
        importe_cobro: solicitud.importe_cobro
          ? String(solicitud.importe_cobro)
          : "",
      });
    }
    ...
    try {
      ...
      if (esFinalizar && !solicitud.pago && pagoMetodoId) {
        const importeEfectivo =
          pagoImporte !== ""
            ? Number(pagoImporte)
            : Number(solicitud.importe_cobro ?? 0);
      ...
      if (esFinalizar) {
        toast.success("¡Servicio finalizado con éxito!", {
          description:
            "La solicitud ha sido completada y cerrada correctamente.",
        });
      }
  ...
  const handleRegistrarPago = async () => {
    ...
    try {
      ...
      setServerError(err?.response?.data?.message || "Error pago.");
    } finally {
      setPagando(false);
    }
  };
  ...
  const onSubmit = async (data: EditFormData) => {
    ...
    try {
      ...
      setServerError(err?.response?.data?.message || "Error update.");
    }
  };
  ...
  const handleCancelar = async () => {
    ...
    toast.warning("¿Cancelar esta solicitud?", {
      description:
        "Esta acción marcará la solicitud como cancelada definitivamente.",
      action: {
        label: "Confirmar",
        onClick: async () => {
          setCancelando(true);
  ...
  if (!solicitud)
    return <p className="p-8 text-center animate-pulse">Cargando...</p>;
